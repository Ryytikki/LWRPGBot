...
class charSheets():
    ...
    async def setToken(self, ctx, *args):
        ...
        for char in charList:
            if char['playerID'] == ctx.message.author:
                char['token'] = token
                urllib.urlretrieve(token, "tokens/{0}.png".format(ctx.message.author.id))
    ...
    @commands.command(pass_context = True)
    async def setSheet(self, ctx, *args):
        global charList
        # Find the character
        for char in charList:
            if char['playerID'] == ctx.message.author:
    ...
    async def mySheet(self, ctx):
        ...
        for char in charList:
            if char['playerID'] == ctx.message.author:
                await self.bot.say("```Name: {0}\nPlayer: {8}\nBio: {6}\nToken: {7}\nSheet: {9}\n\nStats:\nSTR: {1}\nINT: {2}\nCUN: {3}\nLUC: {4}\nBRA: {5}```".format(char['name'], char['str'], char['int'], char['cun'], char['luc'], char['bra'], char['bio'], char['token'], char['playerID'].display_name, char['sheet']))
                return
